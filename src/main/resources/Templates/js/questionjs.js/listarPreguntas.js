document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('http://localhost:8080/questionapi/allquestion',{
    ...
    .then(data => {
        ...
        const encuestaList = document.getElementById('pregunta-list');

        encuestas.forEach(encuesta => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${encuesta.id}</td>
                <td>${encuesta.chapters.chapterTitle}</td>
                <td>${encuesta.questionNumber}</td>
                <td>${encuesta.responseType}</td>
                <td>${encuesta.questionText}</td>
                <td>${encuesta.commentQuestion}</td>
            `;
